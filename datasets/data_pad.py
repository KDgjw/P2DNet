...
class DataloadValTest(Dataset):
    ...
    def __getitem__(self, index):
        ...
        pc_list = np.concatenate(pc_list, axis=0)
        # pcds_target = torch.LongTensor(pc_label_list[0].astype(np.long)).unsqueeze(-1)
        valid_mask_curr = valid_mask_list[0]
        # preprocess
        pcds_xyzi, pcds_coord, pcds_sphere_coord = self.form_batch_tta(pc_list.copy())
        return pcds_xyzi, pcds_coord, pcds_sphere_coord, frame_cur, valid_mask_curr, valid_mask_list, pad_length_list, meta_list_raw
